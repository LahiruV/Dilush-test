...
import { ButtonWidget, DataGrid, ToastManager } from "@peerless/controls";
...
export function EnduserPriceListHistory(props: EnduserPriceListHistoryProps) {
    ...
    mutation.mutate(payload, {
      onSuccess: (response: any) => {
        ...
        if (response) {
          ...
          toast.success('Price list history updated successfully');
        }
      },
      onError: (error: any) => {
        setIsSaving(false);
        // props.msgMgr?.showMessage("error", "Error", "Error occured while updating price list history");
        toast.error('Error occurred while updating price list history');
  ...
  return (
    <div>
      ...
      <ButtonWidget
        id="btn-save-enduser-price-history"
        name="Update Price History"
        classNames="k-button-md k-rounded-md k-button-solid k-button-solid-primary float-right"
        Function={handleSaveClick}
        isDisabled={isSaving}
        isFetching={isEnduserPriceListHistoryDataLoading}
        type="button"
      />
    </div>
  )
