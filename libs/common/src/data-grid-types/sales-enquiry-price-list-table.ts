    ...
    getColumns(): gridColumn {
        return {
            ...
            columns: [
                ...
                { field: 'buyDateFrom', header: 'Deal From', sortable: true, style: {position: 'relative', width: '110px' },  
                body: (rowData: any) => CustomCell({value:  rowData.buyDateFrom ? getFormattedDateTime(rowData.buyDateFrom) : '', styles: { width: '100%', backgroundColor: rowData.colorCode ?? ""} }), filter: false, filterPlaceholder: "Filter By Deal From", filterMatchMode: FilterMatchMode.CONTAINS },
                { field: 'buyDateTo', header: 'Deal To', sortable: true, style: {position: 'relative', width: '110px' }, body: (rowData: any) => CustomCell({value: rowData.buyDateTo ? getFormattedDateTime(rowData.buyDateTo) : '', styles: {width: '100%', backgroundColor: rowData.colorCode ?? ""}}), filter: false, filterPlaceholder: "Filter By Deal To", filterMatchMode: FilterMatchMode.CONTAINS },
                { field: 'dealPrice', header: 'Deal Price', align: 'right', sortable: true, style: {position: 'relative', width: '110px' }, body: (rowData: any) => CustomCell({value:  Number(rowData.dealPrice).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }), styles: { width: '100%', backgroundColor: rowData.colorCode ?? ""}}), filter: false, filterPlaceholder: "Filter By Deal Price", filterMatchMode: FilterMatchMode.CONTAINS },
                { field: 'dealDiscPercent', header: 'Deal Disc $', align: 'right', sortable: true, style: {position: 'relative', width: '110px' }, body: (rowData: any) => CustomCell({value: Number(rowData.dealDiscPercent).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }), styles: {width: '100%', backgroundColor: rowData.colorCode ?? ""}}), filter: false, filterPlaceholder: "Filter By Deal Disc $", filterMatchMode: FilterMatchMode.CONTAINS },
